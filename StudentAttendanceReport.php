...
?>
        ...
        <div id="content" class="clearfix">
            <div class="contentwrapper">
				...
				$POSTCenterId=isset($_POST['CenterId']) ? $_POST['CenterId'] : '';
				$query3="select CenterName, CenterId from vs_centers
				 order by CenterName";
				$check3=mysqli_query($CONNECTION,$query3);
				$SelectedCohort=$ValidSectionId=$ListCurrentCohort=$ShowMonth=$Valid=$count1=$u=$STR=$STRHeading="";
				$DateArray=array();
				while($row3=mysqli_fetch_array($check3))
				{
					$ComboCurrentCohortName=$row3['CenterName'];
					$ComboCurrentCohort=$row3['CenterId'];
					if($POSTCenterId==$ComboCurrentCohort)
					{
					...
					$SelectedCohort="";
					$ListCurrentCohort.="<option value=\"$ComboCurrentCohort\" $SelectedCohort>$ComboCurrentCohortName</option>";
				}
				...
				if($Valid==1 && $ValidSectionId==1)
				{
					...
					$query="select Attendance,Date from studentattendance ";
					...
					$query1="select StudentName,FatherName,Mobile,admission.AdmissionId from registration,admission where
								...
								<form class="form-horizontal" action="" method="post" id="StudentAttendanceReport" name="StudentAttendanceReport">
									<div class="span4">
										<div class="form-row row-fluid">
											<div class="span12">
												<div class="row-fluid">
													<label class="form-label span4" for="CenterId" readonly>Catch Up Center</label>
													<div class="controls sel span8">   
													<select tabindex="1" name="CenterId" id="CenterId" class="nostyle" style="width:100%;" >
													<option></option>
													<?php echo $ListCurrentCohort; ?>
													</select>
													</div>
												</div>
											</div>
									...
									<div class="span4">
										...
										<?php $ButtonContent="Get Student"; ActionButton($ButtonContent,7,"Submit"); ?>
									</div>
								</form>
                            </div>
                        </div>
				...
				<?php } elseif($POSTCenterId=="" && $POSTMonthYear=="") { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#CenterId").select2();
		$('#CenterId').select2({placeholder: "Select"});
		$("#MonthYear").select2();
